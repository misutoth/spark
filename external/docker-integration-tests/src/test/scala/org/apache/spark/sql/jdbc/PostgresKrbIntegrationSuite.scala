...
class PostgresKrbIntegrationSuite extends DockerJDBCIntegrationSuite {
  ...
  val keytabDir = Utils.createTempDir()
  val kdcConf = MiniKdc.createConf
  val kdc = new MiniKdc(kdcConf, kdcDir)
  var postgreKeytabFile: File = _
  ...
  override val db = new DatabaseOnDocker {
    ...
    override def getJdbcUrl(ip: String, port: Int): String =
      ...
      s"jdbc:postgresql://$ip:$port/postgres?gsslib=gssapi&jaasLogin=true"
    override def getStartupProcessName: Option[String] = None

    override def beforeContainerLaunch(containerConfigBuilder: ContainerConfig.Builder,
                                       hostConfigBuilder: HostConfig.Builder): Unit = {
      val dir = Utils.createTempDir()
      copyExecutableResource(dir, "postgres_krb.sh")
      copyResource(dir, "postgresql.conf")
      copyFile(postgreKeytabFile, new File(dir, postgreKeytabFile.getName))

      hostConfigBuilder.binds(
        HostConfig.Bind.from(s"$dir/postgres_krb.sh")
          .to("/docker-entrypoint-initdb.d/postgres_krb.sh").build(),
        HostConfig.Bind.from(s"$dir/postgresql.conf")
          .to("/usr/share/postgresql/postgresql.conf").build()
      )
      // containerConfigBuilder.volumes(s"$dir:/var/lib/postgresql/krbfix/")
      // containerConfigBuilder.entrypoint("/var/lib/postgresql/krbfix/postgres_krb.sh")
    }
  }

  override def beforeAll(): Unit = {
    kdc.start()
    val postgresUser = "postgres/localhost"
    postgreKeytabFile = new File(keytabDir, "postgres.keytab")
    kdc.createPrincipal(postgreKeytabFile, postgresUser)
    logDebug(s"Created keytab file: ${postgreKeytabFile.getAbsolutePath}")
    super.beforeAll()
  }

  private def copyExecutableResource(dir: File, fileName: String) = {
    val newEntry: File = copyResource(dir, fileName)
    newEntry.setExecutable(true, false)
  }

  private def copyResource(dir: File, fileName: String) = {
